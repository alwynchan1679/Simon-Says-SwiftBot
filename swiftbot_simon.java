...
public class swiftbot_simon {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			List<String> sequence = new ArrayList<>(); // stores growing sequence

			while (true) {
				// add a new random light to the sequence
				String newColor = testIndividualUnderlights().get(0);
				sequence.add(newColor);

				System.out.println("Current Sequence: " + sequence);

				// let player try to repeat the sequence
				List<String> playerSequence = PlayerPlays();

				// compare player's input to full sequence
